...
作为自学者，知道何时寻求帮助、如何寻求帮助非常重要。这里提供一个清晰的步骤：

### Level 1: 自己思考 (5-10 分钟)

**做这些**：
1. ✅ 重新仔细阅读题目要求
2. ✅ 在 GHCi 中用简单例子试验（如 `[1]`, `[1,2]`, `[]`）
3. ✅ 画图（特别是递归问题）- 在纸上画出递归调用的每一步
4. ✅ 问自己：
   - 基础情况（base case）是什么？
   - 递归情况如何缩小问题？
   - 我需要用什么函数？（map? filter? 递归?）
...
-- 试试小例子
ghci> myLength []        -- 应该返回？
ghci> myLength [1]       -- 应该返回？
ghci> myLength [1,2]     -- 应该返回？
```

### Level 2: 查看资源 (5-10 分钟)

**做这些**：
1. ✅ 重新阅读讲义中的相关章节
   - 递归问题 → 看"递归"章节
   - map/filter 问题 → 看"高阶函数"章节
2. ✅ 看 TODO 注释中的 Hint（如果有）
3. ✅ 尝试写伪代码或用中文描述算法
   ```
   -- 例如：计算列表长度
   -- 如果列表是空的，长度是 0
   -- 如果列表不是空的，长度是 1 + 剩余列表的长度
   ```
4. ✅ 查看 [调试技巧](lecture.md#10-调试技巧---自学者必备)

### Level 3: 参考答案 (卡住 15+ 分钟后)

**重要**: 如果已经卡住 15 分钟，**不要继续死磕**！

**做这些**：
1. ✅ 查看 `exercises/week-01/solutions/` 中的参考答案
2. ✅ **不要直接复制！** 
3. ✅ 理解解法后，关闭答案文件
4. ✅ 自己重新写一遍（不看答案）
5. ✅ 对比你的解法和参考答案的区别
...
```
❌ 错误做法：看答案 → 复制 → 继续下一题
✅ 正确做法：看答案 → 理解 → 关闭答案 → 自己写 → 对比
```

### Level 4: 寻求帮助

如果看了答案还是不理解，那就是时候问别人了！

**做这些**：
1. ✅ 在 [GitHub Issues](https://github.com/mauyin/haskell-101-cn/issues) 提问
2. ✅ 描述你的思路和尝试过的方法
3. ✅ 贴出你的代码和错误信息
4. ✅ 说明你已经尝试了什么
...
**好问题的例子**：
```
标题：Week 1 Exercise 3.5 - intersperse 函数不理解递归部分

我在做 intersperse 这道题时遇到困难。我的理解是：
- 空列表返回空列表 ✓
- 单个元素返回单个元素 ✓
- 多个元素时，应该在每个元素之间加分隔符
...
但是这样会在最后也加一个分隔符。参考答案用了三个模式匹配，
我不理解为什么需要单独处理 [x] 的情况？
```

### 💡 关键原则

1. **15 分钟规则**：卡住 15 分钟就可以看答案，不要浪费时间
2. **理解 > 完成**：宁可做 10 道理解的题，不要做 25 道复制的题
3. **循序渐进**：难题可以先跳过，做完简单的再回来
4. **反复练习**：看过答案后，过几天再自己做一遍
...
- 🔍 查看 `exercises/week-01/solutions/` 中的参考答案（卡住 15 分钟后）
